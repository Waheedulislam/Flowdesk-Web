...
import { useWorkspaceOverviewAnalytics } from "@/components/workspace/hooks/use-workspace-overview-analytics";
...
export function WorkspacePage() {
  ...
  const [memberForRole, setMemberForRole] =
    React.useState<WorkspaceMemberRecord | null>(null);
  const [memberForRemoval, setMemberForRemoval] =
    React.useState<WorkspaceMemberRecord | null>(null);
  const [invitationForCancellation, setInvitationForCancellation] =
  ...
  const memberState = useWorkspaceMembers({
    accessToken,
    isReady,
    workspaceId,
  });
  const projectsState = useProjects({ accessToken, isReady, workspaceId });
  const analyticsState = useWorkspaceOverviewAnalytics({
    accessToken,
    isReady,
    workspaceId,
  });
  const invitationState = useWorkspaceInvitations({
    accessToken,
    isReady,
    workspaceId,
    enabled: canInvite,
  ...
  React.useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      ...
      setInvitationForCancellation(null);
    }, 0);
  ...
  const headerWorkspace = {
    ...
    description:
      detailState.detail?.description ?? activeWorkspace.description ?? "",
    status: detailState.detail?.status ?? activeWorkspace.status,
    ownerName: detailState.detail?.owner.name ?? "Unavailable",
    ownerEmail: detailState.detail?.owner.email ?? "Unavailable",
  ...
  const cancelInvitation = (invitationId: string) => {
    const invitation = invitationState.invitations.find(
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 pb-16 md:pb-0">
      ...
      <WorkspaceHeader workspace={headerWorkspace} />
      {tab === "overview" ? (
      ...
      {tab === "settings" ? (
        <WorkspaceSettings
          ...
          onWorkspaceLeft={async () => {
            ...
            if (next.length === 0) window.location.assign("/dashboard");
          }}
        />
      ) : null}
      <InviteMemberDialog
        ...
        onInviteSent={() => void invitationState.reload()}
      />
      <CreateWorkspaceDialog open={createOpen} onOpenChange={setCreateOpen} />
      <RoleDialog
        key={memberForRole?.id ?? "role-dialog"}
        member={memberForRole}
        workspaceId={activeWorkspace.id}
        open={Boolean(memberForRole)}
        onOpenChange={(open) => {
          if (!open) setMemberForRole(null);
        }}
      ...
      <RemoveMemberDialog
        ...
        workspaceId={activeWorkspace.id}
        open={Boolean(memberForRemoval)}
        onOpenChange={(open) => {
          if (!open) setMemberForRemoval(null);
        }}
        onSuccess={removeMember}
      />
      <CancelInvitationDialog
        invitation={invitationForCancellation}
        workspaceId={activeWorkspace.id}
        open={Boolean(invitationForCancellation)}
        onOpenChange={(open) => {
          if (!open) setInvitationForCancellation(null);
        }}
        onSuccess={cancelInvitation}
      />
    </div>
  );
}
