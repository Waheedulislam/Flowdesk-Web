...
> = {
  SYSTEM_ADMIN: {
    ...
    stats: [
      ["Total Workspaces", "128", FolderKanban],
      ["Total Users", "2,486", Users],
      ["Active Projects", "342", BarChart3],
      ["Tasks Completed", "18,420", CheckCircle2],
    ],
    sections: [
      "Platform Growth",
      "User Activity",
  ...
  ADMIN: {
    ...
    stats: [
      ["Active Projects", "12", FolderKanban],
      ["Team Members", "24", Users],
      ["Tasks in Progress", "38", ListChecks],
      ["Completion Rate", "82%", CheckCircle2],
    ],
    sections: [
      "Team workload",
      "Project progress",
  ...
  USER: {
    ...
    stats: [
      ["Due Today", "4", ListChecks],
      ["In Progress", "7", Activity],
      ["Completed", "18", CheckCircle2],
      ["Overdue", "2", BarChart3],
    ],
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">{content.copy}</p>
        </div>
      </header>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {content.stats.map(([label, value, Icon]) => (
          <Card key={label}>
            <CardContent className="p-5">
              ...
              <p className="mt-1 text-xs text-success">Updated today</p>
            </CardContent>
          </Card>
        ))}
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        {content.sections.map((title, index) => (
          <Card key={title} className={index === 0 ? "lg:col-span-2" : ""}>
            ...
            <CardContent>
              {index === 0 ? (
                <div className="grid gap-3 sm:grid-cols-3">
                  {["On track", "Needs attention", "Completed"].map(
                    (label, item) => (
